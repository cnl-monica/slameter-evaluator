 ...
 * @author esperian
 */
public class AverageDownloadUploadPacket extends AbstractModule{
    private static CalculateTransferedDataPacket calculate= new CalculateTransferedDataPacket();
    private static AverageDownloadUploadPacket AverageOfTransferredDataPacketInstance;
    
    public static AverageDownloadUploadPacket getInstance(){
        
        
        if(AverageOfTransferredDataPacketInstance==null){
            AverageOfTransferredDataPacketInstance = new AverageDownloadUploadPacket();
            return AverageOfTransferredDataPacketInstance;
        }else{
            return AverageOfTransferredDataPacketInstance;
        }
    }
    
    private AverageDownloadUploadPacket(){ 
    ...
    public void run() {
        ...
        while(true){
                try {    
                    ...
                    Logger.getLogger(AverageDownloadUploadPacket.class.getName()).log(Level.SEVERE, null, ex);
                }
            ...
            try {
                ...
                if(query==null){
                    ...
                    getJedis().publish("ResponseAverageDownloadUploadPacket",ModuleResponse.sentData("AverageDownloadUploadPacket", "error", "Some of required attribute is missing").toString());
                }else{
                ...
                calculate.setEndInterval(time.getLong(1));
                    ...
                    if(calculate.getTimeIntervalInSec()<=1){
                        ...
                        //System.out.println("P"+calculate.getResutValue() +"----"+calculate.getTimeIntervalInSec());
                        finalResult=calculate.getResutValue() /((double) calculate.getTimeIntervalInSec());
                       // System.out.println("|ResponseTopUploader   | Dáta "+finalResult);
                    }
                    getJedis().publish("ResponseAverageDownloadUploadPacket",ModuleResponse.sentDataDouble("AverageDownloadUploadPacket", "ok", finalResult).toString());
                }
    ...
    public String getModuleName() {
        return "AverageDownloadUploadPacket";
    }
