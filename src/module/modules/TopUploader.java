 ...
 * @author esperian
 */
public class TopUploader extends AbstractModule{
    private static CalculateTransferedData calculate= new CalculateTransferedData();
    private static TopUploader topUploaderInstance;
    private MaxValues array;
    
    public static TopUploader getInstance(){
        if(topUploaderInstance==null){
            topUploaderInstance = new TopUploader();
            return topUploaderInstance;
        }else{
            return topUploaderInstance;
        }
    }
    ...
    private TopUploader(){ 
        ...
        setOptionalAttribute("name","source_ip");
    }
    ...
    public void run() {
        ...
        while(true){
            ...
            try {
                ...
                //System.out.println("TopUploader "+startT);
                array = new MaxValues(3);
                ...
                if(query==null){
                    getJedis().publish("ResponseTopUploader",ModuleResponse.sentData("TopUploader", "error", "Some of required attribute is missing").toString());
                }else{
                ...
                calculate.setEndInterval(time.getLong(1));
                    ...
                    DBObject select= new  BasicDBObject("$project",new BasicDBObject("flowId",1).append("flowStartMilliseconds", 1).append("flowEndMilliseconds", 1).append("octetDeltaCount", 1).append("sourceIPv4Address", 1));
                    ...
                    for (DBObject object : outList) {
                        String ipcka = InetAddress.getByAddress((byte[])object.get("sourceIPv4Address")).toString().substring(1, InetAddress.getByAddress((byte[])object.get("sourceIPv4Address")).toString().length());
                    ...
                    //System.out.println("|ResponseTopUploader   | Dáta "+array.toArray().toString());
                    getJedis().publish("ResponseTopUploader",ModuleResponse.sentData("TopUploader", "ok", array.toArray().toString()).toString());
                }
                endT=new Date().getTime();
                
                //System.out.println("TopUploader "+(endT-startT));
